...
/** En-tête de page : version et pitch. */
  ...
  return (
    <header className="border-b border-border/70 pb-6">
      ...
      <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-foreground/80">
        ...
        GeoJSON (RFC 7946), converti depuis des tuiles vectorielles. Trois points
        d&apos;entrée trafic, un catalogue de quartiers, une clé par application.
        Aucune persistance : chaque appel est un instantané best-effort de la ville.
      </p>
    </header>
  )
}
...
/** Sommaire latéral (desktop) + accès à l'explorateur. */
function DevelopersNav() {
  return (
  ...
  return (
    <>
      ...
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        ...
        <code className="font-mono text-primary">/ping</code>) exige une clé API
        dans l&apos;en-tête <code className="font-mono text-primary">X-API-Key</code>.
      </p>
      <Code label="curl">{`curl ${BASE_URL}/ping`}</Code>
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        Dans cette application, le navigateur ne contacte jamais l&apos;API
        directement : un proxy Next.js (
        <code className="font-mono text-primary">/api/predicta/*</code>) détient
        la clé côté serveur et relaie la fraîcheur via les en-têtes{" "}
        <code className="font-mono text-primary">X-Predicta-*</code>.
      </p>
    </>
  )
}
...
function AuthSection() {
  return (
    <>
      ...
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        Une clé API est attribuée à chaque application enregistrée. Les clés sont
        des secrets : elles ne doivent jamais être embarquées dans un client
        public, ni commitées. En cas de clé absente ou invalide, l&apos;API répond{" "}
  ...
  return (
    <>
      ...
      <Endpoint
        method="GET"
        path="/traffic"
        summary="État du trafic live de toute la ville (13 tuiles, ~50 Mo en pratique). Réservé aux usages rares : préférez le quartier ou la zone."
      >
      ...
      <Endpoint
        method="PUT"
        path="/traffic/zone"
        summary="Trafic autour d'un centroïde (disque, 2 tuiles attendues). Le corps porte le centroïde, nom du lieu au plus proche."
      >
        <Code label="body">{`{ "name": "Analakely", "lon": 47.52688, "lat": -18.90793 }`}</Code>
      </Endpoint>
      <Endpoint
        method="GET"
        path="/traffic/quartier/{quartierId}"
        summary="Trafic précis d'un quartier (grille polygonale, 1 à 4 tuiles). Réponse mise en cache 45 s (stale-while-revalidate)."
      >
        <p className="mt-2 text-[12.5px] text-muted-foreground">
          En-têtes : <code className="font-mono text-primary">X-Predicta-Age</code>{" "}
          (ms, si servi du cache),{" "}
          <code className="font-mono text-primary">X-Predicta-Fallback</code>{" "}
          (repli disque centroïde non filtré),{" "}
          <code className="font-mono text-primary">X-Predicta-Partial</code>.{" "}
  ...
  return (
    <>
      ...
      <Endpoint
        method="GET"
        path="/quartiers?q="
        summary="Recherche de quartiers (insensible à la casse, tri alphabétique). q vide ou absent renvoie les 372 quartiers."
      >
        <Code label="réponse">{`[ { "name": "Analakely", "lon": 47.52688, "lat": -18.90793 } ]`}</Code>
        <p className="mt-2 text-[12.5px] text-muted-foreground">
          L&apos;identifiant du quartier (clé de{" "}
          <code className="font-mono text-primary">/traffic/quartier</code>) n&apos;est
          pas exposé ici : il est présent sur chaque segment trafic, dans{" "}
          <code className="font-mono text-primary">properties.quartierId</code>. Le
          catalogue complet (avec ids) est embarqué dans l&apos;application.
        </p>
      </Endpoint>
    </>
  )
...
function GeoJsonSection() {
  return (
    <>
      <H2 id="geojson">GeoJSON</H2>
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        Les réponses trafic sont des FeatureCollections. Un segment est une ligne
        (LineString ou MultiLineString) portant la vitesse observée et le ratio de
        congestion.
      </p>
      <Code label="feature">{`{
  "type": "Feature",
  ...
  "geometry": {
    ...
    "coordinates": [ [47.526, -18.909], [47.529, -18.907] ]
      ...
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        <code className="font-mono text-primary">rate</code> est le ratio vitesse
        observée / vitesse libre (signal de congestion, non dérivable de la
        vitesse seule). Les propriétés{" "}
        <code className="font-mono text-primary">name</code> et{" "}
        <code className="font-mono text-primary">quartierId</code> sont
        optionnelles : la source peut ne pas les fournir pour certains segments.
      </p>
    </>
  )
}
...
function FreshnessSection() {
  return (
    <>
      ...
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        L&apos;API est best-effort : si une tuile échoue, elle est ignorée et la
        réponse reste un GeoJSON valide, signalé par{" "}
        <code className="font-mono text-primary">X-Predicta-Partial</code>. Le cache
        mémoire du quartier sert la donnée pendant sa revalidation (
        <code className="font-mono text-primary">X-Predicta-Age</code>).
  ...
  return (
    <>
      ...
      <p className="text-[13.5px] leading-relaxed text-foreground/80">
        ...
        prédiction : chaque appel est un instantané. Les géométries proviennent
        d&apos;OpenStreetMap (© OpenStreetMap contributors). Les clés sont la seule
        barrière d&apos;authentification — toute logique d&apos;autorisation doit
