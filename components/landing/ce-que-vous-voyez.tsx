...
function animateSectionTitle(section: HTMLElement, title: HTMLElement): void {
  gsap.fromTo(
    title,
...
/** Cards : entrée en stagger avec scale + rotation. */
function animateCards(cards: HTMLElement): void {
  gsap.fromTo(
    cards.querySelectorAll("[data-card]"),
    { opacity: 0, y: 60, scale: 0.9, rotateY: -8 },
    {
      opacity: 1,
      y: 0,
      scale: 1,
...
/** Chaque route SVG se dessine au scroll. */
function animateFeatureRoads(cards: HTMLElement): void {
  cards.querySelectorAll(".feature-road").forEach((road) => {
    ...
    gsap.fromTo(
      ...
      { strokeDasharray: length, strokeDashoffset: length },
      {
        ...
        duration: 1.5,
        ease: "power2.inOut",
        scrollTrigger: { trigger: road, start: "top 85%", once: true },
      }
    )
  })
}

/** Parallaxe subtile des illustrations. */
function animateCardParallax(cards: HTMLElement): void {
  cards.querySelectorAll("[data-card]").forEach((card, index) => {
    gsap.to(card, {
      y: -10 * (index % 2 === 0 ? 1 : -1),
      ease: "none",
...
/**
 * SECTION — Ce que vous voyez. Montre concrètement ce que la carte affiche :
 * cards en stagger + scale, routes SVG dessinées au scroll. Palette cohérente
 * avec le fond papier Predicta (pas de dark cards).
 */
export function CeQueVousVoyez() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (reduced) return
      animateSectionTitle(section, title)
      animateCards(cards)
      animateFeatureRoads(cards)
      animateCardParallax(cards)
    })
    return () => ctx.revert()
  }, [])
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-32">
        ...
        <FeatureGrid cardsRef={cardsRef} />
      </div>
    </section>
  )
}

/** Grille des trois illustrations : carte, état du trafic, informations. */
function FeatureGrid({ cardsRef }: { cardsRef: React.RefObject<HTMLDivElement | null> }) {
  return (
    <div
      ref={cardsRef}
