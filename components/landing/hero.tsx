...
/** Entrée chorégraphiée : voile, wordmark, titre lettre à lettre, CTA, indice. */
function buildEntranceTimeline(refs: EntranceRefs): void {
  gsap
    .timeline({ defaults: { ease: PREDICTA_EASE } })
    .fromTo(refs.veil.current, { opacity: 1 }, { opacity: 0, duration: 1.4, delay: 0.2 })
    .from(refs.wordmark.current, { opacity: 0, y: 30, duration: 0.8 }, "-=0.8")
    .from(
      refs.title.current?.querySelectorAll("span") || [],
      { opacity: 0, y: 40, rotateX: -15, duration: 0.7, stagger: 0.06 },
      "-=0.4"
    )
    .from(refs.subtitle.current, { opacity: 0, y: 20, duration: 0.7 }, "-=0.3")
    .from(refs.cta.current, { opacity: 0, y: 16, scale: 0.95, duration: 0.6 }, "-=0.3")
    .from(refs.scrollHint.current, { opacity: 0, y: -10, duration: 0.5 }, "-=0.1")
}

/** Parallaxe au scroll : le bloc monte, la carte zoome, l'indice s'efface. */
...
/** Voiles de lisibilité : la carte reste visible, le texte se lit partout. */
function HeroBackdrop({ veilRef }: { veilRef: DivRef }) {
  return (
    <>
      <div ref={veilRef} aria-hidden className="pointer-events-none absolute inset-0 z-10 bg-background" />
      <div
        aria-hidden
...
/** Bloc texte : wordmark, titre, sous-titre, appel à l'action. */
function HeroCopy({
  blockRef,
  wordmarkRef,
  titleRef,
  ...
  return (
    <div ref={blockRef} className="pointer-events-none absolute inset-x-0 bottom-0 z-20">
...
/** Indice de défilement, effacé dès les premiers pixels de scroll. */
function ScrollHint({ hintRef }: { hintRef: DivRef }) {
  return (
    <div
      ref={hintRef}
...
/**
 * HERO — Le produit est visible, le mouvement guide l'œil. La carte MapLibre
 * vit en arrière-plan ; l'entrée est chorégraphiée et le bloc quitte l'écran
 * au scroll pendant que la carte reste.
 */
export function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (!reduced) buildScrollEffects(section, block, scrollHintRef)
    })
    return () => ctx.revert()
  }, [])
