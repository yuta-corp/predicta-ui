...
import { congestionLabel, formatRate, formatSpeed } from "@/lib/format"
import type { TrafficFeature } from "@/lib/types/traffic"
import { cn } from "@/lib/utils"

type Rate = TrafficFeature["properties"]["rate"]

const LABEL_CLASS =
  "text-[10px] font-medium uppercase tracking-[0.18em] text-muted-foreground"

/** En-tête : nom de la route + fermeture. */
function RouteHeader({ name, onClose }: { name: string; onClose: () => void }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-border/70 px-5 py-4">
      <div className="min-w-0">
        <p className={LABEL_CLASS}>Route sélectionnée</p>
...
/** Vitesse observée (animée) + niveau de congestion. */
function RouteSpeed({ speed, rate, rateValue }: { speed: string; rate: string; rateValue: Rate }) {
  const congested = typeof rateValue === "number" && rateValue < 0.5
  return (
    <div className="flex items-end justify-between gap-4 px-5 py-4">
      <div>
        <p className={LABEL_CLASS}>Vitesse observée</p>
      ...
      <div className="pb-1 text-right">
        <p className={cn("text-[13.5px] font-medium", congested ? "text-[#c4503a]" : "text-foreground/90")}>
          {congestionLabel(rateValue)}
        </p>
        <p className="mt-0.5 text-[11px] text-muted-foreground">
          {typeof rateValue === "number" ? `Taux ${rate}` : "Taux indisponible"}
        </p>
      </div>
    </div>
  )
...
/** Quartier et source des données. */
function RouteMeta({ quartierName, sourceLabel }: { quartierName: string | null; sourceLabel: string }) {
  return (
    <dl className="grid grid-cols-2 gap-x-4 border-t border-border/70 px-5 py-3.5 text-[12.5px]">
      <div>
        <dt className={LABEL_CLASS}>Quartier</dt>
        <dd className="mt-0.5 truncate text-foreground">{quartierName ?? "—"}</dd>
      </div>
      <div>
        <dt className={LABEL_CLASS}>Données de</dt>
        <dd className="mt-0.5 truncate text-foreground">{sourceLabel}</dd>
      </div>
    </dl>
  )
}

/** Section repliable « Voir le GeoJSON » avec copie. */
function GeoJsonDisclosure({ geoJson }: { geoJson: string }) {
  ...
  return (
    <div className="border-t border-border/70">
      <button
        type="button"
        onClick={() => setShowGeo((value) => !value)}
...
export function RoutePanel() {
  ...
  // Quartier résolu depuis l'id exposé par la feature (index local, zéro requête).
  const quartierName =
  ...
  return (
    ...
    >
      <RouteHeader name={props.name || "Route sans nom"} onClose={() => engine.clearSelection()} />
      <RouteSpeed
        speed={formatSpeed(props.speed)}
        rate={formatRate(props.rate)}
        rateValue={props.rate}
      />
      <RouteMeta
        quartierName={quartierName}
        sourceLabel={selected.label || quartierName || "—"}
      />
      <GeoJsonDisclosure geoJson={JSON.stringify(feature, null, 2)} />
    </aside>
  )
}
...
function AnimatedNumber({ value, className }: { value: string; className?: string }) {
  ...
  const text = useTransform(mv, (v) => (finite ? String(Math.round(v)) : value))
